    ...
    <>
      <button
        ...
        className={`group mt-10 inline-flex w-full items-center justify-center gap-2 rounded-full py-3.5 text-[14.5px] font-medium transition-all
          ${canDeploy && !deploying
            ? 'bg-brand-orange  text-black shadow-[0_10px_40px_-10px_rgba(251,92,28,0.5)] hover:opacity-95'
            : 'cursor-not-allowed bg-white/[0.06] text-black'
          }`}
      >
        {deploying ? (
          <>
          ...
          <>
            Deploy project
            <ArrowUpRight className="h-4 w-4 transition-transform group-hover:-translate-y-[.5rem] group-hover:translate-x-[.5rem]" />
          </>
        )}
      </button>
